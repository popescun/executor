...
# Doxygen's LaTeX tree is scratch: it exists only to be turned into the PDF next to it, so it is
# built under doc/ and removed on the way out, whether this script succeeds or fails.
doxygen_dir="$repo_root/doc/tmp"
latex_dir="$doxygen_dir/latex"
log="$(mktemp -t make_doc)"
trap 'rm -f "$log"; rm -rf "$doxygen_dir"' EXIT
...
# Doxygen creates OUTPUT_DIRECTORY but not a missing parent of it, so make the whole path here
# rather than relying on doc/ already existing.
mkdir -p "$doxygen_dir"
